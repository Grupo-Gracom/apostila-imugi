...
<main>
	...
	<div class="dashboard">
		<div id="unidade" class="box" data-apostila="apostila6" data-unidade="unidade8" data-etapa="conceitos_animacao_02">
			...
			<p>Se desejar mostrar ou ocultar colunas no painel ti meline, clique no botão alternar layers > controles de transferências ou in/out/duration/stretch no canto inferior esquerdo do painel timeline. Pressione Shift +F4 para mostrar ou ocultar a coluna parent. Pressione F4 para alternar as colunas switches e modes. Os resultados de algumas configurações de alternância de layer dependem das configurações dos parâmetros de composição, que estão no canto superior direito do contorno da layer no painel Timeline.</p>
			<ul>
				<li>Vídeo: liga ou desliga as imagens da layer</li>
				<li>Áudio: alterna os sons da layer para ligar ou desligar</li>
				<li>Solo: inclui a layer atual em visualizações e renderização, ignorando layers sem este conjunto de opções</li>
				<li>Bloqueio: bloqueia o conteúdo da layer, evitando todas as alterações (alterna na coluna switches)</li>
				<li>Shy: esconde a layer atual quando a opção de composição hide shy ayers é selecionada</li>
			</ul>

			<h5>Collapse transformations / Continuamente Rasterize</h5>
			<p>Colapsa as transformações se a layer for uma precompose e rasteriza continuamente se for uma layer de forma, layer de texto ou uma layer com um arquivo de vetores como a metragem de origem. Selecionar esta opção para uma layer vetorial faz com que o after indique a camada para cada frame, o que melhora a qualidade da imagem, mas também aumenta o tempo necessário para visualização e renderização.</p>
			<ul>
				<li>Qualidade: alterna entre as opções: best e draft (para qualidade de layer para renderização, incluindo na tela para visualização)</li>
