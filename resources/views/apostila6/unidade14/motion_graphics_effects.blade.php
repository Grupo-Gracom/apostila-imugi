...
<main>
	...
	<div class="dashboard">
		<div id="unidade" class="box" data-apostila="apostila6" data-unidade="unidade14" data-etapa="motion_graphics_effects">
			...
			<ul>
				<li>Novos caminhos são adicionados abaixo de caminhos e grupos existentes.</li>
				<li>Novas operações de caminho - como Zig Zag e Wiggle Paths - são adicionadas abaixo das operações de caminho existentes. Se nenhuma operação do caminho estiver presente, as novas operações do caminho são adicionadas abaixo dos caminhos existentes.</li>
				<li>Novas operações de tinta - traços e preenchimentos - são adicionadas abaixo dos caminhos existentes e acima dos traçados e preenchimentos existentes.</li>
			</ul>
			...
			<ul>
				<li>Caminhos de mesclagem: combina caminhos para um caminho composto.</li>
				<li>Caminhos Offset: expande ou contrai uma forma, compensando o caminho do caminho original. Para um caminho fechado, um valor positivo expande a forma; Um valor negativo contrai a forma. A propriedade Line Join especifica a aparência do caminho onde os segmentos do caminho do offfset se juntam. Uma associação biselada é uma conexão quadrada. Uma junção de mitra é uma conexão pontiaguda. O limite de mitra determina as condições em que uma união biselada é usada em vez de uma junção de mitra. Se o limite de mitra for 4, então, quando o comprimento do ponto atingir quatro vezes o peso do traçado, é usada uma união biselada. Um limite de mitra de 1 provoca uma associação biselada.</li>
				<li>Pucker & Bloat: puxa os vértices de um caminho para fora enquanto curva os segmentos para dentro (Pucker), ou puxa os vértices para dentro enquanto curva os segmentos para fora (Bloat).</li>
				<li>Repetidor: cria várias cópias de uma forma, aplicando uma transformação específica em cada cópia.</li>
				<li>Round Corners: arredonda os cantos dos caminhos. Os valores do raio superior causam maior redondeza.</li>
				<li>Trim Paths: animar as propriedades de início, fifi m e deslocamento para cortar um caminho para criar resultados semelhantes aos resultados obtidos com o efeito Write-on e a configuração Write On para pinceladas. Se a operação do caminho Trim Paths estiver abaixo de vários caminhos em um grupo, então você pode optar por ter os caminhos cortados imultaneamente ou tratados como um caminho composto e cortados individualmente.</li>
				<li>Torção: gira um caminho mais bruscamente no centro do que nas bordas. Inserir um valor positivo gira no sentido horário; inserir um valor negativo gira no sentido anti-horário.</li>
				<li>Wiggle Paths: randomizes (wiggles) um caminho, convertendo-o em uma série de picos irregulares e vales de vários tamanhos. A distorção é auto-animada, o que significa que ela muda ao longo do tempo sem a necessidade de definir quadros-chave ou adicionar expressões. Várias propriedades para esta operação de caminho funcionam da mesma forma que propriedades do mesmo nome para o seletor Wiggly para animação de texto. A propriedade Correlation especifica a quantidade de semelhança entre o movimento de um vértice e o de seus vizinhos; Valores menores criam resultados mais irregulares, já que a posição de um vértice depende menos da posição de seus vizinhos. A propriedade Correlação é semelhante à Correlação para o seletor Wiggly, exceto que a versão Wiggle Paths especifica a correlação entre vértices vizinhos em vez de caracteres vizinhos. Defina o comprimento máximo para caminhos de segmento usando um tamanho absoluto ou relati vo. Defina a densidade das bordas irregulares (Detalhe) e escolha entre bordas macias (Liso) ou bordas afiadas (Canto).</li>
				<li>Wiggle Transform: randomizes (wiggles) qualquer combinação da posição, ponto de ancoragem, escala e transformações de rotação para um caminho. Indique a magnitude desejada do wiggle para cada uma dessas transformações, definindo um valor no grupo de propriedades Transform que está contido no grupo de propriedades Wiggle Transform. As transformações reviradas são auto-animadas, o que significa que elas mudam ao longo do tempo sem a necessidade de definir quadros-chave ou adicionar expressões. A operação Wiggle Transform é especialmente útil após uma operação Repeater, porque permite que você aleatorize as transformações de cada forma repetida separadamente. Várias propriedades para esta operação de caminho se comportam como propriedades do mesmo nome para o seletor Wiggly para animação de texto. A propriedade Correlação especifica a quantidade de semelhança entre as transformações reviradas de uma forma repetida e seu vizinho dentro de um conjunto de formas repeti das. A correlação só é relevante se uma operação Repeater preceder a operação Wiggle Transform. Quando a correlação é 00%, todos os itens repetidos são transformados da mesma maneira; Quando a correlação é 0%, todos os itens repetidos são transformados independentemente.</li>
				<li>Zigzag: converte um caminho em uma série de picos irregulares e vales de tamanho uniforme. Defina o comprimento entre picos e vales usando um tamanho absoluto ou relativo. Defina o número de cristas por segmento de caminho e escolha entre bordas onduladas (Liso) ou bordas irregulares (Esquina).</li>
			</ul>
